...
var checkFile = (req, file, cb) => {
  if (!file.originalname.match(/\.(png|jpg|webp|gif)$/)) {
    return cb("Vui lòng nhập file hình ảnh");
  }
  return cb(null, true);
};
...
router.post("/", upload.single("image"), async (req, res) => {
  try {
    const body = req.body;
    const filename = req.file.filename; // Sửa thành req.file.filename
    const result = await productController.insert({ ...body, image: filename }); // Sử dụng tên file để lưu vào database
