...
router.put("/:id", upload.single("imageUpdate"), async (req, res) => {
  try {
    const { id } = req.params;
    const body = req.body;
    // Kiểm tra xem nếu có tệp hình ảnh đã được tải lên
    if (req.file) {
      // Gán tên tệp hình ảnh gốc vào thuộc tính "image" của đối tượng body
      body.imageUpdate = req.file.originalname;
    }
