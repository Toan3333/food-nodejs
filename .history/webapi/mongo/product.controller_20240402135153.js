...
async function updateById(id, body) {
  try {
    ...
    const { name, price, quantity, description, category } = body;
    ...
    // Xây dựng đối tượng danh mục cập nhật
    ...
    // Sử dụng hình ảnh hiện tại nếu không có hình ảnh mới được chọn
    const imageUpdate = body.image ? body.image : pro.image;

    // Thực hiện cập nhật sản phẩm
    const result = await productModel.findByIdAndUpdate(
      id,
      { name, price, image: imageUpdate, quantity, description, category: cateUpdate },
      { new: true }
    );
