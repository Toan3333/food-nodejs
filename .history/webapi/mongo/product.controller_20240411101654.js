...
async function getProductSpeciel() {
  try {
...
async function insert(body) {
  try {
    const { name, image, price, quantity, category } = body;
  ...
  try {
    ...
    const relatedProducts = await productModel
      .find(
        ...
        // { name: 1, price: 1, quantity: 1, category }
      )
      .limit(3); // Giới hạn số lượng sản phẩm liên quan trả về
