...
formLogin.addEventListener("submit", async function (e) {
  ...
  if (isAdmin) {
    ...
    const loginResult = await loginUser(email, password, 1);
    if (!loginResult) {
      // Nếu đăng nhập không thành công, hiển thị thông báo lỗi và không chuyển hướng
      alert("Đăng nhập không thành công. Vui lòng kiểm tra lại email và mật khẩu.");
      return;
    }
    ...
    // Gọi hàm loginUser để thực hiện đăng nhập, với vai trò tương ứng
    const loginResult = await loginUser(email, password, 0);
    if (!loginResult) {
      // Nếu đăng nhập không thành công, hiển thị thông báo lỗi và không chuyển hướng
      alert("Đăng nhập không thành công. Vui lòng kiểm tra lại email và mật khẩu.");
      return;
