...
async function handleEdit(id) {
  ...
  document.getElementById("nameUpdate").value = currentCategory.name; // Điền tên sản phẩm vào trường input tương ứng
...
async function updateCate(id, newUpdateCate) {
  try {
    const response = await axios.put(`http://localhost:3000/categories/${id}`, newUpdateCate);
    return response.data;
  } catch (error) {
    console.log("Lỗi cập nhật danh muc", error);
    throw error;
  }
}
...
formCategoryEdit.addEventListener("submit", async function (e) {
  ...
  try {
    ...
    // Tìm và cập nhật tên danh mục trong bảng HTML
    const categoryRow = document.querySelector(`#showCategory tr[data-id="${id}"]`);
    if (categoryRow) {
      categoryRow.querySelector("td:nth-child(2)").innerText = nameUpdate;
    }
    closeModalUpdate();
  } catch (error) {
    console.log(error);
