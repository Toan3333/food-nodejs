...
const updateCate = async (id, newUpdateCate) => {
  try {
    const response = await axios.put(`http://localhost:3000/categories`, newUpdateCate);
    return response.data;
  } catch (error) {
...
formCategoryEdit.addEventListener("submit", async function (e) {
  ...
  try {
    const updatedCate = await updateCate(id, { name: nameUpdate }); // Truyền tên danh mục mới vào hàm cập nhật
    showCategory(updatedCate);
    closeModalUpdate();
