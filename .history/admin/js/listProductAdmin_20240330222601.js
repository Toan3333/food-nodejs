...
const showProductAdmin = (item) => {
  if (item && item._id && item.category && item.category.categoryName) {
    const template = `
    <tr>
      <td>${item._id}</td>
      <td>${item.name}</td>
      ...
      <td>$ ${item.price}</td>
      <td>${item.category.categoryName}</td> <!-- Truy cập vào categoryName của đối tượng category -->
      <td>
        <div class="edit-auth">
  ...
  try {
    const addedProduct = await createProduct({
      ...
      category: {
        ...
        categoryName: document.querySelector("#category option:checked").text, // Lấy categoryName từ option được chọn trong dropdown
      },
    });
