...
async function handleEdit(id) {
  ...
  // Kiểm tra dữ liệu danh mục có phải là một mảng không
  if (Array.isArray(currentCategory)) {
    categoryUpdate.innerHTML = "";

    currentCategory.forEach((item) => {
      const tempalte = ` <select name="" id="categoryUpdate" style="padding: 8px">
      <option value="65f50bb9d3ae70c03ccc4530">${item.category.categoryName}</option>
      <option value="65f9011c28564c0f6b279700">${item.category.categoryName}</option>
    </select>`;
    });

    categoryUpdate.value = currentUser.category ? currentUser.category.categoryId : null;
  } else {
    console.log("Dữ liệu danh mục không hợp lệ:", currentCategory);
    // Xử lý dữ liệu không hợp lệ ở đây, ví dụ: thông báo cho người dùng hoặc sử dụng một giá trị mặc định
  }
