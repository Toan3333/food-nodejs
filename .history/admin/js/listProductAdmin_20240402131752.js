...
async function handleEdit(id) {
  ...
  let proupdate = "";
  proupdate += `<select name="" id="categoryUpdate" style="padding: 8px">`;
  cate.map((i) => {
    if (i._id == currentUser.category.categoryId) {
      proupdate += `<option value="65f50bb9d3ae70c03ccc4530">special</option>`;
    }
  });
  //   const cateUp = document.querySelector("#categoryUpdate");
  //   cate.forEach((item) => {
  //     if (item._id == currentUser.category.categoryId) {
  //       const tempalte = `<option value="${item._id}">${item.name}</option>
  // `;
  //       cateUp.insertAdjacentHTML("beforeend", tempalte);
  //     }
  //   });
