...
async function handleEdit(id) {
  ...
  // Lấy đường dẫn hình ảnh từ URL
  const imageUrl = `http://localhost:3000/images/${currentUser.image}`;

  // Tạo request để lấy dữ liệu hình ảnh
  const response = await fetch(imageUrl);
  const blob = await response.blob(); // Chuyển đổi hình ảnh thành dạng blob

  // Tạo đối tượng File mới từ blob
  const file = new File([blob], currentUser.image);

  // Gán đối tượng File vào trường nhập file
  document.getElementById("imageUpdate").files = [file];
}
