...
formPost.addEventListener("submit", async function (e) {
  ...
  try {
    const addedProduct = await createProduct({
      ...
      price: price,
      image: image,
      category: category, // Thay vì "img", bạn nên sử dụng "image" để phù hợp với tên trường trong dữ liệu sản phẩm
    });
    showProductAdmin(addedProduct);
    // Đảm bảo rằng closeModal đã được định nghĩa và hoạt động đúng
    // closeModal();
  } catch (error) {
    console.log(error);
  }
