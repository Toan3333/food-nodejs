...
async function handleEdit(id) {
  ...
  // Tạo một biến để lưu trữ hình ảnh mới
  ...
  imageInput.addEventListener("change", function () {
    ...
    if (file) {
      ...
      updatedImage = file; // Lưu trữ hình ảnh mới
    }
  });

  // Trong formEdit.addEventListener, thêm dòng sau trước khi gửi request:
  formData.append("imageUpdate", updatedImage);
...
formEdit.addEventListener("submit", async function (e) {
  ...
  // Tạo đối tượng FormData và thêm dữ liệu form vào đó
  const formData = new FormData();
  ...
  // Sau khi bạn đã kiểm tra và thêm hình ảnh vào FormData, bạn có thể cập nhật src của thẻ img
  const imagePreview = document.getElementById("imagePreview");
  if (imageUpdate) {
    const reader = new FileReader();
    reader.onload = function (e) {
    ...
    // Nếu không có hình ảnh mới được chọn, bạn có thể giữ nguyên hình ảnh hiện tại
    imagePreview.src = `http://localhost:3000/images/${currentUser.image}`;
  }
