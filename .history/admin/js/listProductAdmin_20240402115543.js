...
function showcategory(item) {
  const template = `<option value="${item._id}">${item.name}</option>`;
...
async function handleEdit(id) {
  try {
    ...
    await getCategory(); // Đảm bảo lấy dữ liệu danh mục trước khi sử dụng

    // Kiểm tra nếu currentCategory không có dữ liệu, nếu không có thì kết thúc hàm
    if (!currentCategory) {
