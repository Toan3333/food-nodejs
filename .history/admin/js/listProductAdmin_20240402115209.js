...
async function handleEdit(id) {
  try {
    ...
    categoryUpdate.innerHTML = ""; // Xóa tất cả các tùy chọn hiện có để tránh trùng lặp

    currentCategory.forEach((item) => {
      // Tạo một tùy chọn mới
      const option = document.createElement("option");
      option.value = item._id; // Đặt giá trị của tùy chọn là ID của danh mục
      option.textContent = item.name; // Đặt nội dung của tùy chọn là tên của danh mục
      // Thêm tùy chọn vào dropdown menu
