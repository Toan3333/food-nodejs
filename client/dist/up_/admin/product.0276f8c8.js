...
})({"3waPX":[function(require,module,exports) {
...
module.bundle.HMR_BUNDLE_ID = "81e611d50276f8c8";
"use strict";
...
},{}],"bUCXd":[function(require,module,exports) {
...
async function getData() {
    try {
        const response = await (0, _axiosDefault.default).get("http://localhost:3000/products");
...
const getProductById = async (id)=>{
    try {
        const response = await (0, _axiosDefault.default).get(`http://localhost:3000/products/${id}`);
...
const showProductAdmin = (item)=>{
    if (item && item._id && item.category && item.category.categoryId && item.category.categoryName) {
        const template = `
    <tr>
      <td>${stt}</td> <!-- Thay \u{111}\u{1ED5}i t\u{1EEB} item.id th\xe0nh item._id -->
      <td>${item.name}</td>
      <td class="admin-img">
      ...
      <td>
        <div class="edit-auth">
          <a href="#" class="edit-btn" data-id="${item._id}">Edit</a> <!-- Thay \u{111}\u{1ED5}i t\u{1EEB} item.id th\xe0nh item._id -->
          <a href="#" class="delete-btn" data-id="${item._id}">Delete</a> <!-- Thay \u{111}\u{1ED5}i t\u{1EEB} item.id th\xe0nh item._id -->
        </div>
      </td>
    </tr>
  `;
    ...
    } else console.log("D\u1EEF li\u1EC7u kh\xf4ng h\u1EE3p l\u1EC7 ho\u1EB7c thi\u1EBFu th\xf4ng tin '_id' ho\u1EB7c 'category'");
};
...
const createProduct = async (NewProduct)=>{
    try {
        ...
        const response = await (0, _axiosDefault.default).post("http://localhost:3000/products", NewProduct);
        return response.data; // Trả về dữ liệu từ phản hồi
    } catch (error) {
        console.log("L\u1ED7i th\xeam s\u1EA3n ph\u1EA9m m\u1EDBi", error);
        throw error; // Ném ra lỗi để xử lý ở nơi gọi
    }
};
const deleteProduct = async (id)=>{
    try {
        const response = await (0, _axiosDefault.default).delete(`http://localhost:3000/products/${id}`);
        const data = response.data.Products;
        return data; // Trả về dữ liệu sản phẩm đã được xóa
    } catch (error) {
        console.log("L\u1ED7i x\xf3a s\u1EA3n ph\u1EA9m", error);
        throw error; // Ném ra lỗi để xử lý ở nơi gọi
    }
};
const updateProduct = async (id, updatedData)=>{
    try {
        const response = await (0, _axiosDefault.default).put(`http://localhost:3000/products/${id}`, updatedData);
        return response.data;
    } catch (error) {
...
async function getCate() {
    try {
        const response = await (0, _axiosDefault.default).get("http://localhost:3000/categories");
...
async function handleEdit(id) {
    ...
    if (cate && Array.isArray(cate)) {
        ...
        let proupdate = "";
        cate.forEach((item)=>{
            if (item._id == currentUser.category.categoryId) proupdate += `<option value="${item._id}" selected>${item.name}</option>`;
            else proupdate += `<option value="${item._id}">${item.name}</option>`;
        });
        categoryUpdateDropdown.insertAdjacentHTML("beforeend", proupdate);
    } else console.log("Kh\xf4ng th\u1EC3 l\u1EA5y danh s\xe1ch danh m\u1EE5c s\u1EA3n ph\u1EA9m.");
...
formEdit.addEventListener("submit", async function(e) {
    ...
    } else // Nếu không có hình ảnh mới được chọn, chỉ cần lưu dữ liệu hiện tại
    saveUpdatedData(id, formData);
});
...
},{"axios":"9PP88","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"gkKU3":[function(require,module,exports) {
...
},{}]},["3waPX","bUCXd"], "bUCXd", "parcelRequiree8ef")

//# sourceMappingURL=product.0276f8c8.js.map
